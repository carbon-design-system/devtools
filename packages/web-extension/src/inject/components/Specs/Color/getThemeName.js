...
function getThemeName (target) {
    ...
    const uiBackground = getComputedStyle(target).getPropertyValue('--cds-ui-background').replace(/ /g, '');
    let theme = false;

    if (uiBackground) {
        // find the matching token among all theme tokens in carbon
        let searchResults = searchCarbonTokens(themes, uiBackground, ['uiBackground'].concat(themeKeys.theme));
        if (searchResults) {
