...
function manageShortcuts () {
    ...
    // shift + ctrl
    if (holdingKeys.indexOf(shift) > -1
     && holdingKeys.indexOf(control) > -1) {
...
function toggleMiniUnitGrid (num) {
    numberShortcut(num, () => toggleDataOptions('toggleGrids', 'toggleMiniUnitGrid')); 
}

function toggleBreakpointLabel (num) {
    numberShortcut(num, () => toggleDataOptions('toggle2xGridOptions', 'toggle2xBreakpoints')); 
}
